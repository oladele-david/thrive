...
class Savings
{
    ...
    public function createSaving($accountId, $amount, $minimumAmount, $savingInterval, $startDate)
    {
        try {
            
            $activeSaving = $this->getActiveSaving($accountId);
            if ($activeSaving) {
                // If there is an active saving, return an error message
                $value_return = array("response" => "error", "title" => "Oops!", "msg" => "You already have an active saving.");
            ...
            $stmt = $this->pdo->prepare("INSERT INTO tb_savings (ref_no, account_id, amount, saving_interval, start_date, minimum_amount, next_removing_date) 
                                         VALUES (:refNo, :accountId, :amount, :savingInterval, :startDate, :minimumAmount, :nextRemovingDate)");
    ...
    public function getActiveSaving($accountId)
    {
        try {
            // Prepare SQL statement to retrieve the active saving for the specified account
            $stmt = $this->pdo->prepare("SELECT * FROM tb_savings WHERE account_id = :accountId AND status = 'active'");
